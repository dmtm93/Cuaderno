...
const SEED_WORDS = [
  ...
  // ---- Género de los sustantivos — reinforcing regular patterns (A1) ----
  ["gato", "el gato — masculine", "Género de los sustantivos", "A1"],
  ["perro", "el perro — masculine", "Género de los sustantivos", "A1"],
  ["casa", "la casa — feminine", "Género de los sustantivos", "A1"],
  ["mesa", "la mesa — feminine", "Género de los sustantivos", "A1"],
  ["hermano", "el hermano — masculine", "Género de los sustantivos", "A1"],
  ["hermana", "la hermana — feminine", "Género de los sustantivos", "A1"],
  ["abuelo", "el abuelo — masculine", "Género de los sustantivos", "A1"],
  ["abuela", "la abuela — feminine", "Género de los sustantivos", "A1"],
  ["manzana", "la manzana — feminine", "Género de los sustantivos", "A1"],
  ["plátano", "el plátano — masculine", "Género de los sustantivos", "A1"],
  ["queso", "el queso — masculine", "Género de los sustantivos", "A1"],
  ["carne", "la carne — feminine", "Género de los sustantivos", "A1"],
  ["cabeza", "la cabeza — feminine", "Género de los sustantivos", "A1"],
  ["brazo", "el brazo — masculine", "Género de los sustantivos", "A1"],
  ["corazón", "el corazón — masculine", "Género de los sustantivos", "A1"],
  ["pie", "el pie — masculine", "Género de los sustantivos", "A1"],
  ["puerta", "la puerta — feminine", "Género de los sustantivos", "A1"],
  ["ventana", "la ventana — feminine", "Género de los sustantivos", "A1"],
  ["flor", "la flor — feminine", "Género de los sustantivos", "A1"],
  ["árbol", "el árbol — masculine", "Género de los sustantivos", "A1"],
  ["playa", "la playa — feminine", "Género de los sustantivos", "A1"],
  ["cielo", "el cielo — masculine", "Género de los sustantivos", "A1"],
  ["luna", "la luna — feminine", "Género de los sustantivos", "A1"],
  ["pez", "el pez — masculine", "Género de los sustantivos", "A1"],
  ["vaca", "la vaca — feminine", "Género de los sustantivos", "A1"],
  ["banco", "el banco — masculine", "Género de los sustantivos", "A1"],
  ["biblioteca", "la biblioteca — feminine", "Género de los sustantivos", "A1"],
  ["iglesia", "la iglesia — feminine", "Género de los sustantivos", "A1"],
  ["hospital", "el hospital — masculine", "Género de los sustantivos", "A1"],

  // ---- Género de los sustantivos — the irregular exceptions (A2) ----
  ["mano", "la mano — feminine (irregular: ends in -o but feminine)", "Género de los sustantivos", "A2"],
  ["agua", "el agua — feminine noun, irregular article (stressed initial \"a\" sound)", "Género de los sustantivos", "A2"],
  ["mapa", "el mapa — masculine (irregular: ends in -a but masculine)", "Género de los sustantivos", "A2"],
  ["síntoma", "el síntoma — masculine (irregular: ends in -a but masculine)", "Género de los sustantivos", "A2"],
  ["índole", "la índole — feminine (irregular: ends in -e)", "Género de los sustantivos", "A2"],
];
...
const CATEGORY_LABELS = {
  ...
  "Género de los sustantivos": "Noun Gender Practice",
...
// Round-robins cards across their categories so new-card introduction mixes
// topics from the start, instead of exhausting one large category (e.g. all
// 29 Números) before a second category ever appears.
function interleaveByCategory(cardsArr) {
...
const DEFAULT_STATS = {
  ...
  newIntroducedCounts: { vocabulario: 0, verbos: 0, frases: 0 },
  dailyNewLimits: { vocabulario: 15, verbos: 15, frases: 15 },
  activeLevels: ["A1"],
  activeDialects: ["latam"],
  cardDirection: "es-first",
  hasSeenGuide: false,
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Migrate the old single global daily limit (if present) into the new
      // per-content-type limits, so a previously customized value carries over
      // instead of silently resetting to the default for all three tabs.
      if (loadedStats && typeof loadedStats.dailyNewLimit === "number" && !loadedStats.dailyNewLimits) {
        const n = loadedStats.dailyNewLimit;
        finalStats.dailyNewLimits = { vocabulario: n, verbos: n, frases: n };
      }
      if (!finalStats.dailyNewLimits) {
        finalStats.dailyNewLimits = { ...DEFAULT_STATS.dailyNewLimits };
      }
      if (!finalStats.newIntroducedCounts) {
        finalStats.newIntroducedCounts = { vocabulario: 0, verbos: 0, frases: 0 };
      }

      // First-ever visit: open the Quick Start Guide automatically rather than
      // relying on someone noticing the header button. Only fires once, ever.
      const isFirstVisit = !finalStats.hasSeenGuide;
      if (isFirstVisit) {
        finalStats = { ...finalStats, hasSeenGuide: true };
        setPanel("guide");
      }
      ...
      if (isFirstVisit) {
        try {
          await window.storage.set("srs-stats", JSON.stringify(finalStats), false);
        } catch (e) {
          setLoadError(true);
        }
      }
  ...
  const newCardsAll = interleaveByCategory(cards.filter((c) => c.status === "new" && inScope(c)));
  const dailyLimitForTab = stats.dailyNewLimits[contentTab] ?? 15;
  const introducedTodayForTab =
    stats.newIntroducedDate === todayStr() ? stats.newIntroducedCounts[contentTab] || 0 : 0;
  const todaysNewAvailable = Math.max(0, dailyLimitForTab - introducedTodayForTab);
  const newCardsForToday = newCardsAll.slice(0, todaysNewAvailable);
  const extraNewCards = newCardsAll.slice(todaysNewAvailable);
  ...
  function startPractice() {
    const queue = shuffle(studiedScopedCards).map((c) => c.id);
  ...
  function studyExtraNewCards(n) {
    const batch = shuffle(extraNewCards.slice(0, n)).map((c) => c.id);
  ...
  function grade(g) {
    ...
    if (wasNew) {
      ...
      if (nextStats.newIntroducedDate === today) {
        nextStats.newIntroducedCounts = {
          ...nextStats.newIntroducedCounts,
          [type]: (nextStats.newIntroducedCounts[type] || 0) + 1,
        };
      } else {
        nextStats.newIntroducedDate = today;
        nextStats.newIntroducedCounts = { vocabulario: 0, verbos: 0, frases: 0, [type]: 1 };
      }
    }
  ...
  function setDailyLimit(type, n) {
    persistStats({ ...stats, dailyNewLimits: { ...stats.dailyNewLimits, [type]: n } });
  }

  function setCardDirection(direction) {
    persistStats({ ...stats, cardDirection: direction });
  }
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 sm:py-10">
        ...
        {panel === "progress" ? (
          ...
          <SettingsView
            ...
            onSetCardDirection={setCardDirection}
                  ...
                  <div className="notecard text-center py-10 px-6">
                    {dueTotal === 0 ? (
                      <>
                        ...
                        <ExtraStudyActions
                          studiedCount={studiedScopedCards.length}
                          extraNewCount={extraNewCards.length}
                          onPractice={startPractice}
                          onStudyExtra={() => studyExtraNewCards(5)}
                        />
                      </>
                    ) : (
                      <>
...
/* Review (SRS) card                                                    */
/* ------------------------------------------------------------------ */
function ExtraStudyActions({ studiedCount, extraNewCount, onPractice, onStudyExtra }) {
  if (studiedCount === 0 && extraNewCount === 0) return null;
  const extraBatch = Math.min(5, extraNewCount);
  return (
    <div
      className="mt-5 pt-5 flex flex-col items-center gap-2"
      style={{ borderTop: "1px dashed rgba(21,74,107,0.2)" }}
    >
      <p className="text-xs mb-1" style={{ color: "var(--ink-soft)" }}>
        Want to keep going?
      </p>
      {studiedCount > 0 && (
        <button className="ink-btn ink-btn-ghost ink-btn-small" onClick={onPractice}>
          Practice {studiedCount} learned card{studiedCount === 1 ? "" : "s"}
        </button>
      )}
      {extraNewCount > 0 && (
        <button className="ink-btn ink-btn-ghost ink-btn-small" onClick={onStudyExtra}>
          Study {extraBatch} more new word{extraBatch === 1 ? "" : "s"} today
        </button>
      )}
    </div>
  );
...
function ReviewCard({ card, flipped, onFlip, onGrade, remaining, done, onExit, direction }) {
  const showSpanishFirst = direction !== "en-first";
  const frontContent = showSpanishFirst ? card.front : card.back;
  const backContent = showSpanishFirst ? card.back : card.front;
  ...
  return (
    <div>
      ...
      <div className="flip-wrap" onClick={onFlip}>
        <div key={card.id} className={`flip-card${flipped ? " is-flipped" : ""}`}>
          <div className="flip-face flip-front">
            <div className="text-xs font-hand text-lg mb-2" style={{ color: "var(--mustard)" }}>
              {categoryLabel(card.category)} · {card.level}
            </div>
            <div className="flex items-center gap-2">
              <div className="font-display text-3xl sm:text-4xl">{frontContent}</div>
              {showSpanishFirst && <SpeakButton text={card.front} />}
            </div>
          ...
          <div className="flip-face flip-back">
            ...
            <div className="flex items-center gap-2">
              <div className="font-display text-3xl sm:text-4xl">{backContent}</div>
              {!showSpanishFirst && <SpeakButton text={card.front} />}
            </div>
            {showSpanishFirst && (
...
function SettingsView({ stats, onToggleLevel, onToggleDialect, onSetDailyLimit, onSetCardDirection, onReset }) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="notecard px-6 py-5">
        <div className="font-display text-lg mb-1">Card direction</div>
        <p className="text-sm mb-3" style={{ color: "var(--ink-soft)" }}>
          Which side of the flashcard shows first — the side you're asked to recall is the harder
          direction.
        </p>
        <div className="flex flex-wrap gap-1.5">
          <button
            className="level-pill"
            data-active={stats.cardDirection !== "en-first"}
            onClick={() => onSetCardDirection("es-first")}
            title="See the Spanish word, recall the English meaning"
          >
            Spanish → English
          </button>
          <button
            className="level-pill"
            data-active={stats.cardDirection === "en-first"}
            onClick={() => onSetCardDirection("en-first")}
            title="See the English meaning, recall the Spanish word"
          >
            English → Spanish
          </button>
        </div>
      ...
      <div className="notecard px-6 py-5">
        ...
        <p className="text-sm mb-3" style={{ color: "var(--ink-soft)" }}>
          New cards per day, set separately for each content tab — Vocabulary tends to move faster
          than Verbs or Phrases, so most people want different limits for each.
        </p>
          ...
          {CONTENT_TYPES.map((t) => (
            ...
            >
              ...
              <input
                ...
                className="ink-input"
                style={{ width: 80 }}
                value={stats.dailyNewLimits[t.code]}
                onChange={(e) => onSetDailyLimit(t.code, Math.max(1, parseInt(e.target.value) || 1))}
              />
            </label>
          ))}
        </div>
